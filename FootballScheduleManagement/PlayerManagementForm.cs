...
namespace FootballScheduleManagement
{
    public partial class PlayerManagementForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void PlayerManagementForm_Load(object sender, EventArgs e)
        {
            ...
            cboClub.DataSource = bsClubManagementForm.GetClubListWithCondition("''");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (flag == true)
            {
                ...
                {
                    bsPlayerManagementForm.AddData(txtName.Text, txtAge.Text, cboPosition.SelectedItem.ToString(), dtpDateOfBirth.Value, cboClub.SelectedValue.ToString(), txtNation.Text);
            ...
            else
            {
                ...
                {
                    bsPlayerManagementForm.UpdateData(txtId.Text, txtName.Text, txtAge.Text, cboPosition.SelectedItem.ToString(), dtpDateOfBirth.Value, cboClub.SelectedValue.ToString(), txtNation.Text);
                }
            }
            btnSave.Enabled = false;
            btnCancel.Enabled = false;
        ...
        private void dgvPlayerList_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            this.cboPosition.Text = dgvClubList.Rows[position].Cells[3].Value.ToString();
            this.dtpDateOfBirth.Value = (DateTime)dgvClubList.Rows[position].Cells[4].Value;
            this.cboClub.Text = bsClubManagementForm.GetSpecificClubName(dgvClubList.Rows[position].Cells[5].Value.ToString()).Rows[0][1].ToString();
            this.txtNation.Text = dgvClubList.Rows[position].Cells[6].Value.ToString();
        }

        public bool CheckEmpty()
        {
            if (txtName.Text == "" || txtAge.Text == "" || cboPosition.SelectedItem == null || dtpDateOfBirth.Value == null || txtNation.Text == "" )
            {
